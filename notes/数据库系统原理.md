...
首先，提交读和可重复读的流程是一样的，只是生成 ReadView 的时间点不同，提交读是为每个快照读都创建一个 ReadView。而可重复读则是执行第一个快照读时创建的，在事务结束前，后面的快照读都会使用当前得到的这个 ReadView。

而 TRX_ID_MAX 应该指的是系统要生成的下一个事务 ID，因为活跃的最高事务 ID 可能已经很旧了，这期间也会提交事务，这些事务应该是可见的。

而对于可见性的判断则应该是：
TRX_ID >= TRX_ID_MAX，表示该数据行快照是在事务启动之后被更改的，因此不可使用。
TRX_ID_MIN <= TRX_ID < TRX_ID_MAX，分成两种情况：

一是如果 TRX_ID 在 TRX_IDs 列表中，则表示这个版本在 ReadView 创建时是未提交的，不可使用。
二是如果 TRX_ID 不在 TRX_IDs 列表中，则表示这个版本在 ReadView 创建时就已经提交了，可以使用。
TRX_ID 如果等于创建该快照的事务 ID，该数据版本也应该是可用的。

在数据行快照不可使用的情况下，需要沿着 Undo Log 的回滚指针 ROLL_PTR  找到下一个快照，再进行上面的判断。

/*需要注意的是*/
事务的提交顺序和事务的 ID 无关。我们可以假设这样一种情况，在可重复读级别隔离下，在开启事务执行第一个快照读前，系统中活跃的事务号为{2, 4, 5}，事务 3 已提交。
现在我们开启事务执行快照读，进行可见性判断时，如果按照原文的规则，事务 3 应该是不可见的，但是实际应该为可见的，因为事务 3 在事务开启前就已经提交。
